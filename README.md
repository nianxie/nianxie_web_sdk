...
1. `npm run nx:package`
   ...
   - 若工程根目录存在 `schema.json` 或 `schema/schema.json`，会将 `schema.json` 注入到 `dist.zip` 的 `dist/` 根目录
   - 若工程根目录存在 `config.json` 或 `config/config.json`，会将 `config.json` 注入到 `dist.zip` 的 `dist/` 根目录
...
- `npm run nx:package`
  ...
  - 可选 JSON 注入：若命中以下候选路径，会把对应文件以 `dist/schema.json`、`dist/config.json` 写入压缩包（与 `dist/index.html` 同级）
    - schema: `./schema.json` 或 `./schema/schema.json`
    - config: `./config.json` 或 `./config/config.json`
